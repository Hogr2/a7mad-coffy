...
import { formatIQD, getProduct } from "@/lib/products";
...
function CartPage() {
  ...
  const placeOrder = () => {
    const lines = [
      "طلب جديد من موقع الضوء ☕",
      "",
      ...rows.map(
        (r) => `• ${r.product.name} (${r.item.grind}) × ${r.item.qty} = ${formatIQD(r.product.price, r.item.qty)} د.ع`,
      ),
      "",
      `المجموع: ${subtotal.toLocaleString("ar-EG")} د.ع`,
      `الشحن: ${shipping === 0 ? "مجاني" : `${shipping.toLocaleString("ar-EG")} د.ع`}`,
      `الإجمالي: ${total.toLocaleString("ar-EG")} د.ع`,
      "",
      `الاسم: ${form.name}`,
      `الهاتف: ${form.phone}`,
      `المدينة: ${form.city}`,
  ...
  if (placed) {
    return (
      <div className="mx-auto max-w-lg px-6 py-24 text-center">
        ...
        <h1 className="mt-8 font-display text-4xl">فتحنا لك واتساب</h1>
        <p className="mt-4 text-muted-foreground">
          شكرا {form.name || "لك"}. اضغط إرسال بالواتساب حتى يوصلنا طلبك، وراح نتصل بك خلال ساعتين لتأكيد التوصيل.
        </p>
        ...
        <div className="mt-12 grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7">
            <ul className="divide-y divide-border">
              {rows.map(({ item, product }) => (
                <li key={`${item.id}|${item.grind}`} className="flex gap-4 py-6">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <div className="text-xs text-brass mt-1">الطحن: {item.grind}</div>
                      </div>
                      <button onClick={() => remove(item.id, item.grind)} className="text-muted-foreground hover:text-destructive" aria-label="أزل">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="mt-auto flex items-center justify-between pt-4">
                      <div className="flex items-center border border-border rounded-sm">
                        <button onClick={() => setQty(item.id, item.grind, item.qty - 1)} aria-label="أنقص" className="w-10 h-10 flex items-center justify-center hover:bg-secondary"><Minus className="w-3 h-3" /></button>
                        <span className="w-8 text-center font-mono text-sm">{item.qty}</span>
                        <button onClick={() => setQty(item.id, item.grind, item.qty + 1)} aria-label="زد" className="w-10 h-10 flex items-center justify-center hover:bg-secondary"><Plus className="w-3 h-3" /></button>
                      </div>
                      <div className="font-mono text-brass">{formatIQD(product.price, item.qty)} <span className="text-[10px] text-muted-foreground">د.ع</span></div>
                    </div>
                  </div>
                </li>
              ))}
          ...
          <aside className="lg:col-span-5 lg:sticky lg:top-24 h-fit border border-border rounded-sm p-6 sm:p-8 bg-card grain">
            ...
            >
              ...
              <Field label="الهاتف" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} required dir="ltr" type="tel" pattern="[0-9+][0-9\s-]{8,}" title="أدخل رقم هاتف صحيح" />
              ...
              <button type="submit" className="w-full mt-4 rounded-sm bg-brass hover:bg-brass-soft text-primary-foreground py-3.5 text-sm font-medium transition">
                أكمل الطلب عبر واتساب · الدفع عند الاستلام
              </button>
              <p className="text-[11px] text-muted-foreground text-center">
                يفتح واتساب برسالة جاهزة فيها تفاصيل طلبك — فقط اضغط إرسال.
              </p>
            </form>
          </aside>
        </div>
...
function Field({ label, value, onChange, required, dir, type = "text", pattern, title }: { label: string; value: string; onChange: (v: string) => void; required?: boolean; dir?: "ltr" | "rtl"; type?: string; pattern?: string; title?: string }) {
  return (
