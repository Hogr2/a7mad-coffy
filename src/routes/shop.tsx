  ...
  return (
    <div className="mx-auto max-w-7xl px-6 sm:px-10 py-16 sm:py-24">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-10 border-b border-border/60 pb-6">
        <span className="font-mono text-[11px] uppercase text-muted-foreground me-2">تحميص:</span>
        {roasts.map((r) => (
          <button
            key={r}
            onClick={() => setFilter(r)}
        ...
        {list.map((p, i) => (
          ...
          >
            <div className="relative aspect-[4/5] overflow-hidden rounded-sm bg-secondary grain">
              ...
              <div className="absolute top-4 start-4 font-mono text-[10px] uppercase text-brass-bright">
                #{String(i + 1).padStart(2, "0")} · {p.roast}
              </div>
              <div className="absolute bottom-4 end-4 font-mono text-[10px] uppercase text-cream/80">
                {p.weight}
              </div>
            </div>
